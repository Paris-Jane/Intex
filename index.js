...
// EVENT REGISTRATION FUNCTIONALITY (USER END):
    ...
    app.post('/register-event/:eventId', async (req, res) => {
        ...
        try {
            ...
            const startGcal = eventDate.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, '');
            
            eventDate.setHours(parseInt(endTime[0]), parseInt(endTime[1]), 0);
            const endGcal = eventDate.toISOString().replace(/[-:]/g, '').replace(/\.\d{3}/, '');
    ...
    app.get('/profile/:id', async (req, res) => {
        ...
        try {
            ...
            const participant = await knex('participants')
                ...
                .select('participants.participant_id','participants.participant_email','participants.participant_first_name','participants.participant_last_name',
                    'participants.participant_dob','participants.participant_role','participants.participant_phone','participants.participant_city',
                    'participants.participant_state','participants.participant_zip','participants.participant_school_or_employer','participants.participant_field_of_interest',
            ...
            const milestones = await knex('milestones').where('participant_id', participantId).select('milestone_id', 'milestone_title', 'milestone_date', 'milestone_category').orderBy('milestone_date', 'desc');
            
            // Get donations sorted by most recent first
            const donations = await knex('donations').select('donation_id', 'donation_date', 'donation_amount').orderBy('donation_date', 'desc');
            ...
            const eventRegistrations = await knex('event_registrations')
                ...
                .select('event_registration_id','participant_id','event_id','registration_status','registration_attended_flag','registration_created_at_date',
                    'registration_created_at_time','registration_check_in_date','registration_check_in_time'
                )
                .orderBy([
            ...
            const surveys = await knex('survey_results as sr')
                ...
                .select(
                    'sr.survey_id','sr.event_registration_id','sr.survey_satisfaction_score','sr.survey_usefulness_score',
                    'sr.survey_instructor_score','sr.survey_recommendation_score','sr.survey_overall_score','sr.survey_nps_bucket',
                    'sr.survey_comments','sr.submission_date','sr.submission_time'
                )
                .orderBy([
...
// EDIT FUNCTIONALITY
    ...
    app.get("/edit/:table/:id", async (req, res) => {
        ...
        knex.select().from(table_name).where(primaryKey, id).first().then((info) => {
